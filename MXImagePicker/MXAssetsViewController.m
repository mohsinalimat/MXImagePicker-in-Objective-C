...
#import "MXAssetsViewController.h"
...
@interface MXImagePickerController (Private)
...
- (NSArray *)mx_indexPathsFromIndexesWithSection:(NSUInteger)section
{
...
- (NSArray *)mx_indexPathsForElementsInRect:(CGRect)rect
{
...
@interface MXAssetsViewController () <PHPhotoLibraryChangeObserver, UICollectionViewDelegateFlowLayout>
...
@implementation MXAssetsViewController
...
{
    if ([self.imagePickerController.delegate respondsToSelector:@selector(mx_imagePickerController:didFinishPickingAssets:)]) {
        [self.imagePickerController.delegate mx_imagePickerController:self.imagePickerController
...
- (void)updateSelectionInfo
{
    ...
    if (selectedAssets.count > 0) {
        ...
        if (selectedAssets.count > 1) {
            format = NSLocalizedStringFromTableInBundle(@"assets.toolbar.items-selected", @"MXImagePicker", bundle, nil);
        } else {
            format = NSLocalizedStringFromTableInBundle(@"assets.toolbar.item-selected", @"MXImagePicker", bundle, nil);
        }
...
- (void)updateFetchRequest
{
    if (self.assetCollection) {
        ...
        switch (self.imagePickerController.mediaType) {
            case MXImagePickerMediaTypeImage:
                options.predicate = [NSPredicate predicateWithFormat:@"mediaType == %ld", PHAssetMediaTypeImage];
                break;
                
            case MXImagePickerMediaTypeVideo:
...
- (void)updateCachedAssets
{
    ...
    if (delta > CGRectGetHeight(self.collectionView.bounds) / 3.0) {
        ...
        [self computeDifferenceBetweenRect:self.previousPreheatRect andRect:preheatRect addedHandler:^(CGRect addedRect) {
            NSArray *indexPaths = [self.collectionView mx_indexPathsForElementsInRect:addedRect];
            [addedIndexPaths addObjectsFromArray:indexPaths];
        } removedHandler:^(CGRect removedRect) {
            NSArray *indexPaths = [self.collectionView mx_indexPathsForElementsInRect:removedRect];
            ...
            if (![collectionChanges hasIncrementalChanges] || [collectionChanges hasMoves]) {
                ...
                [self.collectionView performBatchUpdates:^{
                    ...
                    if ([removedIndexes count]) {
                        [self.collectionView deleteItemsAtIndexPaths:[removedIndexes mx_indexPathsFromIndexesWithSection:0]];
                    }
                    
                    NSIndexSet *insertedIndexes = [collectionChanges insertedIndexes];
                    if ([insertedIndexes count]) {
                        [self.collectionView insertItemsAtIndexPaths:[insertedIndexes mx_indexPathsFromIndexesWithSection:0]];
                    }
                    
                    NSIndexSet *changedIndexes = [collectionChanges changedIndexes];
                    if ([changedIndexes count]) {
                        [self.collectionView reloadItemsAtIndexPaths:[changedIndexes mx_indexPathsFromIndexesWithSection:0]];
                    }
                } completion:NULL];
            }
...
{
    MXAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"AssetCell" forIndexPath:indexPath];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    if (kind == UICollectionElementKindSectionFooter) {
        ...
        switch (self.imagePickerController.mediaType) {
            case MXImagePickerMediaTypeAny:
            {
                NSString *format;
                if (numberOfPhotos == 1) {
                    if (numberOfVideos == 1) {
                        format = NSLocalizedStringFromTableInBundle(@"assets.footer.photo-and-video", @"MXImagePicker", bundle, nil);
                    } else {
                        format = NSLocalizedStringFromTableInBundle(@"assets.footer.photo-and-videos", @"MXImagePicker", bundle, nil);
                    }
                } else if (numberOfVideos == 1) {
                    format = NSLocalizedStringFromTableInBundle(@"assets.footer.photos-and-video", @"MXImagePicker", bundle, nil);
                } else {
                    format = NSLocalizedStringFromTableInBundle(@"assets.footer.photos-and-videos", @"MXImagePicker", bundle, nil);
                }
            ...
            case MXImagePickerMediaTypeImage:
            {
                ...
                NSString *format = NSLocalizedStringFromTableInBundle(key, @"MXImagePicker", bundle, nil);
...
{
    if ([self.imagePickerController.delegate respondsToSelector:@selector(mx_imagePickerController:shouldSelectAsset:)]) {
        PHAsset *asset = self.fetchResult[indexPath.item];
        return [self.imagePickerController.delegate mx_imagePickerController:self.imagePickerController shouldSelectAsset:asset];
    }
...
{
    MXImagePickerController *imagePickerController = self.imagePickerController;
    ...
    if (imagePickerController.allowsMultipleSelection) {
        ...
        if ([imagePickerController.delegate respondsToSelector:@selector(mx_imagePickerController:didFinishPickingAssets:)]) {
            [imagePickerController.delegate mx_imagePickerController:imagePickerController didFinishPickingAssets:@[asset]];
        }
    }
    
    if ([imagePickerController.delegate respondsToSelector:@selector(mx_imagePickerController:didSelectAsset:)]) {
        [imagePickerController.delegate mx_imagePickerController:imagePickerController didSelectAsset:asset];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didDeselectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([imagePickerController.delegate respondsToSelector:@selector(mx_imagePickerController:didDeselectAsset:)]) {
        [imagePickerController.delegate mx_imagePickerController:imagePickerController didDeselectAsset:asset];
    }
}
